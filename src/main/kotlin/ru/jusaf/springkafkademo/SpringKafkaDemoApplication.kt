...
/**
 * У сообщения (message) есть key и value
 * Когда мы читаем сообщения, мы делаем коммит сообщения.
 * Commit - значит, мы успешно прочитали сообщение. Возвращает кафке брокеру. Позиция, прочитанных сообщений в партиции,
 * называется offset-ом. Мы прочитали нулевое сообщение, тогда offset = 1, следующее прочитаем 1.
 * Если один groupId, больше повторных нет, consumer забирает полностью партицию одну.
 * Мы читаем не одним сообщением, а группой сообщений. Batch, мы читаем бачами, а не по одному сообщению.
 * На жестком диске хранятся данные партиции и сообщения.
 * Если мы читаем бачами, либо всё читаем, либо ничего. Как буква А в ACID.
 * Другой консюмер может начать с начала читать сообщения, если даже второй консюмер его прочитал.
 * Retention - это условие, по которому элемент с кафки удаляется. 7 дней по умолчанию.
 * В партиции мы можем тоже много что делать.
 * Redis - key, value
 */